...
app = Flask(__name__, template_folder='frontend/tempates', static_folder='frontend/static')  # Fixed typo: tempates -> templates
...
def preprocess_image_for_model(img_file, target_size):
    ...
    try:
        ...
        # Open and process image exactly like Kaggle code
        img = Image.open(img_file.stream)
        img = img.convert('RGB')  # Ensure RGB format
        img = img.resize(target_size, Image.Resampling.LANCZOS)  # High-quality resize
        
        # Convert to array and normalize (0-1 range) - matches ImageDataGenerator(rescale=1./255)
...
def get_ensemble_predictions(img_file):
    ...
    try:
        ...
        # Get predictions from each model with correct input sizes
        ...
        # Create ensemble prediction (average of all models) - EXACTLY like Kaggle
...
def get_top_predictions(predictions, model_confidences=None, top_k=5):
    ...
    for idx in top_indices:
        ...
        # Add individual model confidences if provided
...
def predict():
    ...
    try:
        ...
        # Get ensemble predictions using the FIXED method
...
def search_animals():
    ...
    try:
        ...
        for animal_name, details in ANIMAL_DETAILS.items():
            # Search in animal name, scientific name, and description
            search_fields = [
                animal_name.lower(),
...
def health_check():
    ...
    try:
        ...
        return jsonify({
            ...
            'total_classes': len(ANIMAL_CLASSES),
            'class_order': ANIMAL_CLASSES  # Added for debugging
        })
    except Exception as e:
